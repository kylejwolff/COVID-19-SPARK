...
object Queries {
    ...
    def usAllPercentByQuarter(spark: SparkSession, covid_df: DataFrame, uid_df: DataFrame): DataFrame = {
        ...
        val usq = covid_df.select(covid_df("country"), covid_df("region"), covid_df("observed_date"), covid_df("confirmed"), covid_df("deaths"), covid_df("recovered"))
        .withColumn("confirmed", col("confirmed").cast("int"))
        .withColumn("deaths", col("deaths").cast("int"))
        .withColumn("recovered", col("recovered").cast("int"))
        .groupBy("region", "country", "observed_date").max("confirmed", "deaths", "recovered")
        .select("country", "observed_date", "max(confirmed)", "max(deaths)", "max(recovered)")
        .groupBy("country", "observed_date").sum("max(confirmed)", "max(deaths)", "max(recovered)")
    ...
    def globalPercentByQuarter(spark: SparkSession, covid_df: DataFrame, uid_df: DataFrame): DataFrame = {
        ...
        val gq = covid_df.select(covid_df("country"), covid_df("region"), covid_df("observed_date"), covid_df("confirmed"))
        .withColumn("confirmed", col("confirmed").cast("int"))
        .groupBy("region", "country", "observed_date").max("confirmed")
        .select("country", "observed_date", "max(confirmed)")
        .groupBy("country", "observed_date").sum("max(confirmed)")
